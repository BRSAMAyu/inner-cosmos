...
## 第六增量（2026-09-12 晚）：时间窗硬约束检索语义（检查点 26）

1. **`TimeWindowParser`**（`ai/retrieval`，纯函数）：查询中的**可度量**时间表达解析为硬窗口——今天/今晚→1 天、这周/本周→7、本月→31、上个月→62、上周→14、最近一个月→31、最近N天/周/月（中文数字+阿拉伯）→N×单位、今年/最近一年→366
2. **刻意的不对称语义**：光杆"最近"**不开窗**——自然语言里它常常描述"回想"而非记忆本身（"我最近老是想起以前导师说的话"），硬过滤会恰好藏起用户想要找回的旧材料；光杆最近保持既有新鲜度软信号
3. **接线**：`MemoryRetrievalServiceImpl.retrieve()` 候选加载后、计分前——窗口外候选（按 lastTouchedAt??createdAt）整体排除，构造性不可中
4. **`TimeWindowSemanticsTest` 3/3**：13 组解析断言（含光杆最近=null、今年≠今天次序坑）；可度量窗口对强词法重叠（否则必中）的 700 天旧记忆硬排除；光杆最近下旧记忆仍可召回且新近排前（软信号语义保持）
5. **bank v1.2.0**：新增 MR-013 time_window_hard_exclusion（"这周"查询 + 700 天强重叠旧记忆被构造性排除），SHA 重冻结（8c28446d…），13 案例全过；后端全量回归 **1584/1584** 绿
