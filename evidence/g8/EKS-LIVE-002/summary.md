...
- Cluster **MyEKS**, Kubernetes **1.36**, platform **eks.6**, status ACTIVE, account `<redacted-account-id>`, us-east-1.
...
- Cross-built an **amd64** image (the arm64 dev image can't run on amd64 nodes) via a thin runtime Dockerfile
  ...
  pushed to ECR: `<redacted-account-id>.dkr.ecr.us-east-1.amazonaws.com/inner-cosmos:eks-dev`
...
- **External ingress**: Gateway `Programmed=True`, HTTPRoute `Accepted`+`ResolvedRefs`; a real **AWS ELB**
  (`<redacted-elb-hostname>.us-east-1.elb.amazonaws.com`) serves the app —
