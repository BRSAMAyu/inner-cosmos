...
class StructuredAiServiceBadOutputTest {
    ...
    @Test
    void remoteOnlyGreetingCannotBeForcedIntoMockAbBucket() {
        when(llmClient.chat(any(LlmRequest.class))).thenReturn("{\"score\": 5}");

        ScoreResult result = service.call(1L, "AURORA_PROACTIVE_GREETING_DAILY_TALK",
                "grounded greeting", Map.of("requireRemoteProvider", true),
                ScoreResult.class, ScoreResult::new);

        ArgumentCaptor<LlmRequest> captor = ArgumentCaptor.forClass(LlmRequest.class);
        verify(llmClient).chat(captor.capture());
        assertEquals(5, result.score);
        assertNotEquals(Boolean.TRUE, captor.getValue().forceMock,
                "a personalised greeting must never be generated by MockLlmClient");
        verify(abTestService).recordMetrics(eq(1L), eq("REMOTE"),
                eq("AURORA_PROACTIVE_GREETING_DAILY_TALK"), anyDouble(), eq(true), eq(false));
    }
}
