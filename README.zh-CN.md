...
[最新候选版本：v0.1.0-rc.1](https://github.com/BRSAMAyu/inner-cosmos/releases/tag/v0.1.0-rc.1) · Apache-2.0 · 已签名多架构 OCI（含 SBOM 与 provenance）

[English README](README.md) · [评委指南](docs/competition/JUDGE-GUIDE.md) · [验证快照](docs/competition/VERIFICATION-SNAPSHOT-2026-08-01.md) · [文档导航](docs/README.md) · [Agent 入口](CLAUDE.md) · [机器验收账本](docs/goal/complete-product-acceptance.yml)
...
首个稳定外部纵切面已发布为 [OpenAPI 3.1 v1](src/main/resources/static/openapi/inner-cosmos-v1.yml)，运行时地址为 `/openapi/inner-cosmos-v1.yml`。签入的 TypeScript 声明通过 `cd web && pnpm run api:generate` 生成，`api:check` 与 `api:diff` 分别阻断漂移和破坏性变更。新客户端应优先使用其中覆盖的 `/api/v1` 认证、Aurora、共鸣体、慢信与 Persona 路由。核心写请求要求 `Idempotency-Key`，共鸣体边界更新还要求 `If-Match`，Aurora 恢复使用 `Last-Event-ID`。迁移期间保留旧 `/api`；在所有公共域、分页和跨 Pod 实时 SSE 完成前，验收账本仍诚实保持 `IN_PROGRESS`。
...
Inner Cosmos 采用 [Apache License 2.0](LICENSE)。除非文件另有声明，仓库原创代码、文档与资产
均按该许可证提供；第三方依赖和随附材料继续适用其各自许可证。
