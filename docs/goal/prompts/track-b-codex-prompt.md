...
严格遵守 Track B 文件所有权，不编辑 Java、数据库迁移、AI Lab 或全局账本；后端需求写入 `docs/goal/tracks/track-b-integration-requests.yml`，可以使用明确标记、最终必须移除的 typed fixture/adapter。真实密钥只由环境变量/本机忽略文件注入。采用持续 Goal 循环推进，每个可恢复纵向 checkpoint 提交 Git，更新 `track-b-status.yml` 和 `evidence/track-b/`；小步跑相关测试，在阶段和 PR 前运行任务书规定的完整门禁。允许推翻旧 UI/UX 和调整技术方案，但保护用户数据、安全边界、可访问性和可恢复证据。

不要在一次提交、一次绿测、上下文压缩或普通困难处结束。直到所有机器可执行 Track B 验收完成、工作树干净、证据与 PR 描述齐备，或只剩不可伪造的设备/签名/账号人工门禁时，才输出最终交接结果并创建/推送 PR（若账号权限允许；否则给出精确命令和阻塞）。
