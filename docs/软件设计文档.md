...
- **在本项目中的应用（项目里有三组独立的策略族）**：
  1. **对话模式**：`ModeStrategy`（`ai/mode/ModeStrategy.java:7`）定义 `segment()`（注入的提示片段）与 `temperature()`。实现含 `DailyTalkStrategy`（`ai/mode/DailyTalkStrategy.java`，温度 0.85）、`SocraticStrategy`、`ThoughtClarifyStrategy`、`ActionSplitStrategy`、`SleepReviewStrategy`、`RelationReviewStrategy`。`PromptBuilder.withModeSegment(strategy)`（`PromptBuilder.java:85`）按当前策略注入片段与温度。
  2. **共鸣体人格 / Agent 回复**：`AgentReplyStrategy`（`ai/strategy/AgentReplyStrategy.java:3`，`reply(input)`），实现含 `QuietLibrarianStrategy`、`SeasideWatchmakerStrategy`、`PassionatePainterStrategy`、`ZhuangziSeedStrategy`、`StoicSeedStrategy` 等十余种人格。
  3. **供应商优选**：`FailoverLlmClient.orderedCandidates()`（`ai/client/FailoverLlmClient.java:71`）按请求里的 `preferredProvider` 动态调整供应商尝试顺序，本质也是一种选择策略。
- **为什么这样设计**：不同对话模式 / 不同人格本质是「同一行为的不同实现」。用策略 + 注册表，可以让产品随时新增一种模式或一个共鸣体人格而不动核心对话流程，温度等参数也随策略走。
- **关键代码角色**：`ModeStrategy` / `AgentReplyStrategy` = Strategy 接口；各具体 Strategy = ConcreteStrategy；`ModeRegistry` / 调用方 = Context。
...
1. **Mock 优先的 AI 架构**：`MockLlmClient` / `MockAsrClient` 与真实供应商共用同一接口（适配器），默认离线即可跑通全部 AI 链路（`LlmConfig.java:287` 缺 key 时回退 mock）。这让评测、单测、无网环境都能复现完整体验，也是 A/B 实验 `forceMock` 的基础。
...
7. **多供应商容错（Failover）**：`FailoverLlmClient`（`ai/client/FailoverLlmClient.java:12`）按候选顺序逐个尝试，任一供应商抛异常即切下一个，全挂才抛 `AiProviderException`，保障线上可用性。
