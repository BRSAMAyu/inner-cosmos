...
5. **There is no real front door, and the out-of-the-box demo config was wrong (now fixed).** Every core page silently auto-logs a brand-new visitor into the shared `demo` account (**M-009**, still open), AND the shipped LLM default (`prod`/`minimax`) contradicted the no-key demo promise (**RT-01** — now fixed to `dev`/`mock`, chat verified 200). For a privacy-first product the auto-login is both a clarity failure and a data-exposure smell.
...
|----|-----|---------|----------|-----|
| **RT-01** | **P1 ✓fixed** | **Shipped LLM default (prod/minimax) broke the no-key demo — FIXED & verified.** | `application.yml:99-100` shipped `mode=prod`+`provider=minimax`; prod disables fallback; all keys 401 at boot → proactive job `AiProviderException`. Contradicted README/AGENTS ("默认 Mock / 无 Key 可演示"). **Fixed:** defaulted `mode=dev`+`provider=mock`; **chat `/api/aurora/message` verified 200** (ASCII + UTF-8). *Honesty note:* my original "chat→500 reproduced" was a curl GBK-encoding artifact (`Invalid UTF-8 middle byte` before any LLM call); the config defect was real (proactive job confirmed) but I over-claimed the chat symptom. | Applied `dev`/`mock`; verified 200. Reconcile README/AGENTS copy (config now matches docs). |
...
> **RT-01 (fixed):** the shipped `prod`/`minimax` default contradicted the no-key demo promise and made LLM-dependent calls fail (proactive job `AiProviderException` at boot). Now defaulted to `dev`/`mock`; chat verified 200. (My initial "chat→500" was a curl GBK artifact — the config defect was real, the symptom was over-claimed.)
