...
> 数据源真相：`src/main/resources/schema.sql` + `com.innercosmos.config.Schema*Initializer` 一系列启动期迁移 + `@TableName` 实体类。
...
`capsule_id`（`FK CASCADE`）/ `memory_card_id`（`FK CASCADE`）/ `abstract_excerpt` / `authorization_status`（默认 AUTHORIZED）。共鸣体只能访问脱敏摘录，不直连原始记忆。
