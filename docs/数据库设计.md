...
`capsule_id`（`FK ON DELETE CASCADE`）/ `allow_topics` / `blocked_topics` / `max_conversation_turns` / `allow_letter_request` / `privacy_level` / `version`。`version` 从 1 单调递增，是 v1 API 的 ETag/`If-Match` 比较并交换权威；陈旧写入返回 409，不能覆盖较新的边界决定。
