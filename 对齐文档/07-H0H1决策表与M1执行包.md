...
> 适用关系：本文件继续负责 W0 安全、工程基线和云原生验收；Aurora、Self/Emergence、proactive、共鸣体、用户模型、心理 Skill 和创新 Demo 的产品/AI 架构以 `08-Aurora生命感与共鸣智能创新架构.md` 为准。
...
| H0-02 | Aurora 只能作为反思协作者，Aurora Self / Constitution / Emergence 只能下沉为后台 Behavior Policy | SUPERSEDED_BY_08 | Aurora 双核、Self Genome、人格/关系演化和用户可感知生命感以 `08` 为准 |
| H0-03 | 用户确认与纠正高于模型推断 | FROZEN | 权威顺序必须进入数据模型、检索和测试 |
| H0-04 | M1 只交付 Aurora → Memory Receipt，创新能力不得进入展示主线 | SUPERSEDED_BY_08 | Memory Receipt 仍是数据控制基础；最终展示必须加入 Aurora 生命感、时间/多消息/打断、Self、共鸣体或匹配的可感知创新纵切 |
...
| H0-09 | 共鸣体从静态卡片/有限问答起步，主动消息采用极低固定预算，M1 后默认保守收缩创新 | SUPERSEDED_BY_08 | 共鸣体高拟真编译、宽松交互、Temporal Proactive、匹配与 Simulator 以 `08` 为准 |
...
与上述方向冲突的旧产品约束已由 `08-Aurora生命感与共鸣智能创新架构.md` 正式取代。后续工程必须保持 W0 安全结果，同时按照 `08` 增强而非压低既有产品语义；若两者发生实质冲突，必须停止对应修改并提交人类决策。

### 1.1 功能组合

| 立即强化 | 受控实验 | M1 延后 |
|---|---|---|
| Aurora 文字/语音、多消息/等待/打断、proactive 时间唤醒、Self/Emergence、Memory/User Model、动态共鸣体、匹配纵切、心理 Skill、Memory Receipt、洞察型星空 | 慢信、周报、更多匹配策略、Simulator B2B、完整 Skill 市场 | Capacitor、完整 React、大规模社交图、复杂生产推荐平台、新加坡真实发行 |
...
- 全量 React 重写；
- 大规模生产推荐平台、广泛社交、复杂运营后台；高拟真动态共鸣体与至少两种匹配策略的 Demo 纵切由 `08` 进入创新计划；
