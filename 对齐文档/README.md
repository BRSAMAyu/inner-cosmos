# Inner Cosmos 对齐文档权威索引

> 本文件回答三个问题：Agent 应先读什么、文档冲突时听谁的、如何从愿景追溯到代码与证据。

## 0. 2026-09-05 商业化目标补充：优先中国大陆

用户已明确要求从参赛作品转向真实商业产品，并确认“中国大陆优先”。涉及本轮商业化目标时，在读取原总目标后，先读[中国大陆商业化执行蓝图](../docs/commercialization/01-中国大陆商业化执行蓝图.md)与[商业化完全体愿景](../docs/commercialization/02-商业化完全体愿景.md)：64个主工作包、13个阶段子门、20项愿景承诺相互追溯。

本轮市场、中文体验、国内数据路径、生产成本、监管／商店、运营和商业验证以这两份新规格为准；与旧新加坡首发、课堂Demo优先或展示成本策略冲突时，新商业规格优先。下文L0及24／25的历史工程事实、已验证能力和安全授权边界继续保留，不把旧PASS解释成大陆生产PASS。CP-01负责执行启动时逐项迁移旧69项验收及人工门；这次文档交付本身不修改旧账本完成状态，也不授权真实发布、收款或外部账户操作。

下方“当前／唯一执行权威”描述原课堂／完全体收敛战役；不覆盖本节明确的商业化目标补充。新法规、真实代码和用户后续明确指令仍优先于任何计划文本。

## 1. 权威层级

| 层级 | 文档 | 作用 |
|---|---|---|
| L0 | `/goal-objective.md` | 唯一总目标、完成定义、自主权限与人类门禁 |
| L1 | `09`—`12` | 完全体产品、工程、UIUX、实施与验收的目标规范 |
| L1-EXEC | `16-体验优先的完全体重构策略与产品战役.md` | 体验优先裁决、重构授权、五场产品战役与验证节奏 |
| L1-EXEC | `17-单会话持续Goal模式执行协议.md` | 单顶层会话的恢复、循环、优先级、状态持久化和唯一终止条件 |
| L1-EXEC | `19-双轨并行完全体收敛与交接计划.md` | 当前双 PR 的目标、边界、文件所有权、质量门槛、合并与最终收尾协议 |
| L1-EXEC-CURRENT | 24-完全体最终收敛与云原生课程战役.md | 当前唯一执行权威：四分支集成、产品完全体、UIUX、云原生高级能力、协作与最终验收 |
| L1-AI | `08-Aurora生命感与共鸣智能创新架构.md` | Aurora、记忆画像、共鸣体、心理 Skill 的详细创新架构 |
| L1-DEPLOY | `14-AWS-Academy-EKS约束与双轨部署架构.md` | 课程 Learner Lab 与商业新加坡环境的边界、替代适配器和双轨演示合同 |
| L2 | `00`、`02`—`07` | 项目理解、EKS、移动端、新加坡、Agent 流程、技术演进、近期决策与执行基线 |
| L3-CURRENT | `20-当前状态重对账与完全体差距基线.md` | 基于当前 HEAD 复核的已完成能力、剩余差距、风险与下一阶段裁决 |
| L3-CURRENT-AUDIT | `docs/audit/2026-07-23-gemini-master-audit-reconciliation.md` | 对 Gemini 36 项审查线索的当前 HEAD 独立裁决、正确修复合同与 W0V 硬验收 |

| L3 | `01`、`13`、`15`、`docs/audit`、`docs/gap_analysis`、现有 specs | 历史时点的评估、连接指南、进展审查、缺口和专题设计；用于事实发现，不自动决定最终方向 |
| L4 | work package、ADR、research、evidence、测试报告 | 实施契约、局部决策与可复现证据 |
| 历史 | 根目录旧总纲、旧推进书、旧 UIUX 资产 | 设计来源与历史上下文；未被上层吸收的内容不具有目标裁决权 |

## 2. 冲突裁决

按以下次序处理冲突：

1. 安全事实、法律事实和当前代码行为必须以实时验证及有效证据为准，不能被愿景文本覆盖。
2. “最终要做成什么”以 L0、L1 和 L1-AI 为准。
3. 同层冲突时，范围更专门、日期更新、且明确写出替代关系的文档优先。
4. `08` 与旧文档在 Aurora 主动性、Self/Constitution/Emergence、动态共鸣体、匹配、慢信上的创新方向优先于旧的保守降级建议。
5. 当前实现与目标不一致时，应登记 gap 并迁移，不得反向修改目标来合理化旧实现。
6. 新发现若会改变核心产品承诺或技术边界，先写 ADR/RFC，并同步更新 L1 文档与验收清单，避免隐性漂移。

## 3. 完全体规范

| 编号 | 文件 | 核心回答 |
|---|---|---|
| 09 | `09-完全体产品愿景与功能规格.md` | 为谁解决什么问题，完整产品包含什么，体验效果如何 |
| 10 | `10-完全体系统架构与工程规格.md` | 用什么架构实现，模块、数据、AI、API、云和移动端怎样协作 |
| 11 | `11-完全体UIUX与交互设计规格.md` | 用户如何感知和操作产品，视觉、交互、响应式和可访问性怎样统一 |
| 12 | `12-完全体实现路线与最终验收规约.md` | Agent 怎样自主推进，什么证据代表真正完成 |
| 14 | `14-AWS-Academy-EKS约束与双轨部署架构.md` | 如何在受限 Academy EKS 中真实展示云原生，同时保留本地完整体和商业新加坡目标 |
| 16 | `16-体验优先的完全体重构策略与产品战役.md` | 如何摆脱旧原型束缚，以 Living Aurora 等产品战役自主推进并打磨最终体验 |
| 17 | `17-单会话持续Goal模式执行协议.md` | 如何让 Goal 在同一会话跨提交和上下文压缩持续运行，直到完全体或只剩人类门禁 |
| 18 | `18-组员与Coding-Agent启动部署交接指南.md` | 如何让组员或 Coding Agent 在不泄密、不混淆三种 profile 证据的前提下快速启动、部署、验证和恢复 |
| 19 | `19-双轨并行完全体收敛与交接计划.md` | 如何把剩余工作拆成低冲突的智能数据轨与完整体验交付轨，并用两个 PR 收敛 |
| 20 | `20-当前状态重对账与完全体差距基线.md` | 当前真实完成度、关键瓶颈、隐藏缺口和“效果完成”的判据 |
| 21 | 21-双轨阶段合并审查与完全体续跑交接.md | A/B checkpoint 合并事实、组员恢复点、剩余完全体路线与统一验收交接 |
| 22 | 22-最终集成与教师演示候选状态.md | 最终本地集成、演示候选验证、开放的人类/外部门禁与最新恢复点 |
| 23 | 23-组员PR集成审查与本地演示收口.md | 组员 PR #5–#7 的集成审查、修正及最新网站/API/Android 验收与设备边界 |
| 24 | 24-完全体最终收敛与云原生课程战役.md | 产品体验与云原生课程证明双主线、锁定分支、五大战役和最终合法终态 |
| 25 | 25-云原生高级能力展柜与课程评分设计.md | 三环境的 Foundation/Hero/Extension Gallery、插件选型、波次、评分和课程证据包 |

机器可读对应物：`docs/goal/complete-product-acceptance.yml`。当前执行状态只取 `docs/goal/closure-campaign-state.yml`；23、release-candidate-state、22、21、旧 single-session/teammate state、历史双轨拆分和 `docs/tracks/` 仅用于追溯，不得提供当前 cursor。

## 4. 专题文档阅读路线

- Aurora、主动性、连续自我、共鸣体、心理建模：先读 `08`，再读 `06` 的 AI 章节和对应 innovation work packages。
- Kubernetes、EKS、运行角色、Day-2：先读 `10`，再读 `02`、`06`。
- 高级 Kubernetes/CNCF 插件、课程包装与展示评分：先读 `24`，再读 `25`，环境限制服从 `14`。
- AWS Academy Learner Lab、SQS/存储/IAM 限制和课程演示：先读 `14` 与 `docs/goal/aws-academy-capabilities.yml`；`13` 只作为连接操作参考。
- App、PWA、Capacitor、推送与移动权限：先读 `11`，再读 `03`。
- 新加坡发行、PDPA、跨境 LLM、商店、HSA、税务和支付：先读 `04`，最终由 `12` 的发布门禁验收。
- 近期安全和基线工作：读 `07`、`evidence/m1` 及最新 Git/CI 结果。
- 自主研发流程：读 `12`，再用 `05` 获取更细的 Agent 协作和质量环设计。
- 当前实现优先级、是否允许推翻旧界面/架构、产品战役与测试频率：读 `16`；其执行裁决优先于旧的微切片或兼容性习惯。
- 单会话连续自主执行：读 `17`，但当前 cursor 只取 `docs/goal/closure-campaign-state.yml`；内部提交与检查点不得结束顶层 Goal。
- 组员接管启动、local-complete 或 AWS Academy 部署：读 `18`；其中的操作步骤必须服从 `14` 的环境能力边界。
- 当前最终收敛接管：先读 24、`docs/goal/closure-campaign-state.yml` 与验收账本；23 及更早文档仅用于追溯，不得重复旧 WIP 或把本地/Academy 证据夸大为商业 PASS。
- 外部 Agent 审查线索：先读原报告定位，再以 `docs/audit/2026-07-23-gemini-master-audit-reconciliation.md` 裁决；不得把“VICTORY CONFIRMED”等标签代替当前 HEAD 复现或反证。

## 5. 文档与实现的双向追溯

每个新能力应形成以下链路：

```text
产品承诺（09/11）
  → 架构责任与数据契约（10/08）
  → Goal/Work Package/ADR
  → 代码、迁移、IaC、测试与评测
  → evidence
  → complete-product-acceptance.yml 状态
```

禁止出现只有愿景没有验收、只有代码没有产品承诺、只有单测没有真实体验、只有 demo 没有安全与恢复证据的“伪完成”。

## 6. 文档维护规则

- L1 文档描述稳定目标和跨模块契约，不记录每日流水账。
- 具体执行拆到 `docs/work-packages`；关键取舍写 ADR；实验写 `docs/research`；结果写 `evidence`。
- 任何验收状态必须指向真实文件、命令输出或人工签字，不得仅写“已完成”。
- 当前态变化后更新机器清单和事实基线；愿景只有在产品决策确实变化时更新。
- 中文是主要设计语言；用户可见文案、API 错误和产品内容必须逐步形成中英双语资产。
