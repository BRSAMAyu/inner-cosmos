...
| 模块化 | ArchUnit + 领域接口/事件；Spring Modulith 可选 | 当前 ADR-0003 采用 ArchUnit；仅在 package-by-feature 演进后再以 Modulith 增强，不以未采用框架冒充完成 |
...
| 消息 | EventTransport + transactional outbox/inbox | `local/academy` 使用 JDBC outbox；`commercial-sg` 使用 SQS/DLQ；Kafka 仅作有证据的扩展 |
...
不要求一次性搬目录。当前先用 package、ArchUnit 和公开接口形成真实边界；只有完成 package-by-feature 演进并证明收益后才采用 Spring Modulith。每个领域模块拥有自己的应用服务、领域对象、repository port、事件和 API mapper；禁止其他模块直接使用其 mapper 或修改其表。
...
Kubernetes 同时承担生产工程与课程技术展示。API、worker、scheduler、migration 的拆分是基础；在其上按 `24` 建设业务指标弹性、跨异步 trace、渐进发布、策略即代码、网络隔离/GitOps/运行时安全/混沌/成本等能力。扩展插件可高于当前规模刚需，但必须由独立 profile 隔离、可关闭、有运行证据，并保持 Academy fallback。
