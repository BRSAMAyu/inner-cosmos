...
> **当前最终集成交接**：完整阅读 [对齐文档/22-最终集成与教师演示候选状态.md](对齐文档/22-最终集成与教师演示候选状态.md) 与 [docs/goal/release-candidate-state.yml](docs/goal/release-candidate-state.yml)。21、19、20、teammate-continuation-state 与 `docs/tracks/` 保留为历史拆轨证据；不得 cherry-pick 旧 WIP 或重复实现。真实 Provider、设备、法务和生产账户仍是人工门禁。
