...
describe("SocialGroupsView", () => {
  it("shows an empty state and lets the owner create a new group", () => {
    ...
    render(<SocialGroupsView groups={[]} invites={[]} friends={[]} selectedGroupId={null} members={[]} createBusy={false} isInviteBusy={() => false} isInviteDecisionBusy={() => false} isLeaveBusy={() => false} currentUserId={1}
  ...
  it("lists groups, selects one, invites a friend and shows members", () => {
    ...
    render(<SocialGroupsView groups={[group()]} invites={[]} friends={[friend()]} selectedGroupId={1}
      members={[{ userId: 1, memberRole: "OWNER", nickname: "我" }]} createBusy={false} isInviteBusy={() => false} isInviteDecisionBusy={() => false} isLeaveBusy={() => false} currentUserId={1}
  ...
  it("shows pending invites and lets the invitee accept or decline", () => {
    ...
    render(<SocialGroupsView groups={[]} invites={[invite]} friends={[]} selectedGroupId={null} members={[]} createBusy={false} isInviteBusy={() => false} isInviteDecisionBusy={() => false} isLeaveBusy={() => false} currentUserId={1}
  ...
  // shared `busy` boolean previously disabled EVERY group action (create/invite/accept/decline/
  // leave) the moment any ONE of them was in flight. Two unrelated invites must be independently
  // disable-able: isInviteDecisionBusy is keyed by memberId, so invite 9 being busy must never
  // disable invite 11's accept/decline buttons, and must never disable the unrelated create-group
  // button either (a completely different action).
  it("keys invite-decision busy state by memberId -- an unrelated invite's buttons and the create-group button stay enabled", () => {
    const invites: GroupInvite[] = [{ memberId: 9, groupId: 2, groupName: "读书会" }, { memberId: 11, groupId: 3, groupName: "跑步小组" }];
    render(<SocialGroupsView groups={[]} invites={invites} friends={[]} selectedGroupId={null} members={[]}
      createBusy={false} isInviteBusy={() => false} isInviteDecisionBusy={memberId => memberId === 9} isLeaveBusy={() => false} currentUserId={1}
      onSelectGroup={() => undefined} onCreateGroup={() => undefined} onInvite={() => undefined}
      onRespondInvite={() => undefined} onLeaveGroup={() => undefined} />);
    const acceptButtons = screen.getAllByRole("button", { name: "接受" });
    const declineButtons = screen.getAllByRole("button", { name: "婉拒" });
    expect(acceptButtons[0]).toBeDisabled(); // invite 9 -- busy
    expect(declineButtons[0]).toBeDisabled();
    expect(acceptButtons[1]).not.toBeDisabled(); // invite 11 -- unrelated, must stay enabled
    expect(declineButtons[1]).not.toBeDisabled();
    fireEvent.change(screen.getByPlaceholderText("群组名字"), { target: { value: "x" } });
    expect(screen.getByRole("button", { name: "创建群组" })).not.toBeDisabled(); // unrelated action entirely
  });
  ...
  it("lets a member leave the selected group", () => {
    ...
    render(<SocialGroupsView groups={[group()]} invites={[]} friends={[]} selectedGroupId={1}
      members={[{ userId: 2, memberRole: "MEMBER", nickname: "我" } satisfies GroupMember]} createBusy={false} isInviteBusy={() => false} isInviteDecisionBusy={() => false} isLeaveBusy={() => false} currentUserId={2}
      onSelectGroup={() => undefined} onCreateGroup={() => undefined} onInvite={() => undefined}
  ...
  it("hides the leave button for the group owner, since the server always rejects it (regression: 2.2.4)", () => {
    render(<SocialGroupsView groups={[group({ ownerUserId: 1 })]} invites={[]} friends={[]} selectedGroupId={1}
      members={[{ userId: 1, memberRole: "OWNER", nickname: "我" } satisfies GroupMember]} createBusy={false} isInviteBusy={() => false} isInviteDecisionBusy={() => false} isLeaveBusy={() => false} currentUserId={1}
      onSelectGroup={() => undefined} onCreateGroup={() => undefined} onInvite={() => undefined}
      onRespondInvite={() => undefined} onLeaveGroup={() => undefined} />);
