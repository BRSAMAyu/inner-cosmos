...
async function gotoCosmos(page: import("@playwright/test").Page) {
  ...
  // Heart diary lives behind the "今日记录与心声" cosmos sub-tab (doc 24 section 3.3: ported
  // legacy modules must not stack vertically in one page).
  await page.getByRole("navigation", { name: "内宇宙分区导航" }).getByRole("button", { name: "今日记录与心声", exact: true }).click();
